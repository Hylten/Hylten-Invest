...
function fixFalseHeadings(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (headingMatch) {
      ...
      // Let's also check if it's a heading merged with a paragraph
      // e.g., "# Heading Text. Sentence start..."
      // If it contains a period followed by space, we can split them!
      ...
      if (isLowerStart || hasMultipleSentences || isLongSentence || isFullSentence || containsBullets) {
...
function fixInlineBullets(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('•')) {
      ...
      const newLines = [];
      if (firstPart) {
        // Remove trailing bold markers if any
...
function fixInlineNumberedLists(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Split inline numbered list like "1. Map... 2. Identify..."
    if (/\d+\.\s+/.test(line) && !line.startsWith('1. ') && !line.startsWith('2. ') && !line.startsWith('3. ')) {
      // Split by numbers
...
function normalizeSpacing(content) {
  ...
  // Fix single-newline paragraph breaks
  let result = content;
  ...
  // Ensure headers have blank line after them
...
function removeJunkFooters(content) {
  ...
  // Remove common junk patterns at end of file
  // Pattern: lines containing only ": $5M+" type text
  const lines = result.split('\n');
  
  // Work backwards from end, removing junk lines
  while (lines.length > 0) {
    ...
    // Also check if the last real line ends with junk
...
function simpleHash(text) {
  // Create a simple word-frequency fingerprint
...
for (const file of files) {
  ...
  // 1. Normalize frontmatter author
